...
* При каждом **следующем** клоне вашего репозитория, если такие вдруг будут, необходимо добавлять базовый (из которого был создан ваш) в качестве upstream с помощью следующей команды: 
  ...
  ```
  Обратите внимание, что нужно использовать ссылку для ssh вида `git@github.com:CPP-KT/task-repo.git`, а не https версию. При инициализации репозитория этот шаг делает за вас `init-repo.sh`.
