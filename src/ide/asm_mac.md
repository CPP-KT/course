...
```
Для практики по ассемблеру, вам понадобится `nasm`, а также дебаггеры `edb` и `gdb`, установите их при помощи следующей команды:
```bash
sudo apt install nasm binutils edb-debugger gdb xterm
```
